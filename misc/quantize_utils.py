...
"""def k_means_cpu(weight, n_clusters, init='k-means++', max_iter=50):
    ...
    if n_clusters > weight.size()[0]:
        n_clusters = weight.size()[0]
...
class QModule(nn.Module):
    def __init__(self, w_bit=-1, a_bit=-1, half_wave=True):
        ...
        # self.writer=writer = SummaryWriter('runs/QModule')
    ...
    def _quantize_activation(self, inputs):
        if self._quantized and self._a_bit > 0:
            ...
            if self._trainable_activation_range:
                # self.writer.add_histogram(str(self.__class__)+'QModule_before_quantization', inputs.detach().cpu().numpy(), bins='auto')
                ...
                # self.writer.add_histogram(str(self.__class__)+'QModule_after_quantization', inputs.detach().cpu().numpy(), bins='auto')
            else:
                if self._half_wave:
...
class QConv2d(QModule):
    ...
    def forward(self, inputs):
        # self.writer.add_histogram(str(self.__class__)+'Qconv2d_before_quantization', inputs.detach().cpu().numpy(), bins='auto')

        inputs, weight, bias = self._quantize(
            inputs=inputs, weight=self.weight, bias=self.bias
        )

        # self.writer.add_histogram(str(self.__class__)+'Qconv2d_after_quantization', inputs.detach().cpu().numpy(), bins='auto')
...
class QLinear(QModule):
    def __init__(
        ...
        super(QLinear, self).__init__(w_bit=2, a_bit=2, half_wave=half_wave)
    ...
    def forward(self, inputs):
        ...
        return F.linear(inputs, weight=self.weight, bias=self.bias)
...
if __name__ == "__main__":
    model = QConv2d(in_channels=3, out_channels=4, kernel_size=3)
    print(model.w_bit)
    input = torch.rand(1, 3, 9, 9)
    # output=model(input)
    # x=nn.Conv2d(3,4,3)
    # x.weight=nn.Parameter(x.weight/1000)

    print(model.weight)
    # q=QModule(w_bit=8)
    # input1,weight,bias=q._quantize(input,x.weight,bias=None)
    # print(weight)
    output = model(input)
