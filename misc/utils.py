...
def load_teacher_checkpoint(config, model, logger):
    logger.info(
        f"==============> Resuming form {config.MODEL.TEACHER_RESUME}...................."
    )
    if config.MODEL.TEACHER_RESUME.startswith("https"):
        checkpoint = torch.hub.load_state_dict_from_url(
            config.MODEL.TEACHER_RESUME, map_location="cpu", check_hash=True
        )
    else:
        with open(config.MODEL.TEACHER_RESUME, "rb") as fp:
            checkpoint = torch.load(fp, map_location="cpu")
