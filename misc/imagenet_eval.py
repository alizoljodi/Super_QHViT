...
def validate_one_subnet(
    ...
    for batch_idx, (images, target) in enumerate(val_loader):
        ...
        # compute output
        output = subnet(images, writer, batch_idx)
        if print_act_range == False:
            # print(list(subnet.parameters()))
            print_act_range = True
        ...
        if args.distributed and getattr(args, "distributed_val", True):
            ...
            stats = torch.tensor([corr1, corr5, loss, batch_size], device=device)
