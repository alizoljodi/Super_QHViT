...
class AttentiveNasStaticModel(MyNetwork):
    ...
    def forward(self, x, writer=None, batch_idx=None, block_id=None):
        ...
        for i, block in enumerate(self.blocks):
            if isinstance(block, MobileInvertedResidualBlock):
                x = block(x, writer, batch_idx, i)
            else:
                x, k_q_v_dict = block(x, writer, batch_idx, i)
                k_q_v_dict_list.append(k_q_v_dict)

            if writer is not None and batch_idx is not None:
                writer.add_histogram(
                    str(block.__class__) + str(i),
                    x.detach().cpu().numpy(),
                    global_step=batch_idx,
                    bins="auto",
                )
...
class AttentiveQNasStaticModel(MyNetwork):
    def __init__(
        self, first_conv, blocks, last_conv, classifier, resolution, use_v3_head=True
    ):
        super(AttentiveNasStaticModel, self).__init__()
