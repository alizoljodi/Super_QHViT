...
from .modules.dynamic_layers import (
    ...
    DynamicQConvBnActLayer,
    DynamicLinearLayer,
    DynamicShortcutLayer,
    DynamicMBQConvLayer,
    DynamicQShortcutLayer,
    DynamicQConvBnActLayer,
    DynamicQLinearLayer,
)
...
from .modules.transformer import (
    DynamicQSwinTransformerBlock as StandardDynamicQSwinTransformerBlock,
)
from .modules.transformer import (
...
class AttentiveNasDynamicModel(MyNetwork):
    def __init__(self, supernet, n_classes=1000, bn_param=(0.0, 1e-5), parent=None):
        ...
        for stage_id, key in enumerate(self.stage_names[1:-1]):
            ...
            for i in range(n_block):
                ...
                if (
                    stage_id >= 3 and i >= 1
                ):  # and False: # not (stage_id == 4 and i == 0):
                    rescale = 1.0
                    transformer = StandardDynamicSwinTransformerBlock(
                        ...
                        rescale=rescale,
                        shift=False,
                    )  # StandardDynamicQSwinTransformerBlock(feature_dim, input_resolution=(18,18), num_heads=num_heads[stage_id-3], window_size=18, shift_size=0, mlp_ratio=1., act_layer=act_func, rescale=rescale, shift = False ,w_bit=8, a_bit=8, half_wave=True) #StandardDynamicSwinTransformerBlock(feature_dim, input_resolution=(18,18), num_heads=num_heads[stage_id-3], window_size=18, shift_size=0, mlp_ratio=1., act_layer=act_func, rescale=rescale, shift = False ) #StandardDynamicQSwinTransformerBlock(feature_dim, input_resolution=(18,18), num_heads=num_heads[stage_id-3], window_size=18, shift_size=0, mlp_ratio=1., act_layer=act_func, rescale=rescale, shift = False ,w_bit=8, a_bit=8, half_wave=True)
                    ...
                    mobile_inverted_conv = DynamicMBConvLayer(
                        ...
                        ),  # ,dw_w_bit=8,dw_a_bit=8,pw_a_bit=8,pw_w_bit=8,
                    )
    ...
    def forward(self, x):
        ...
        for stage_id, block_idx in enumerate(self.block_group_info):
            ...
            for idx in active_idx:
                ...
                if isinstance(self.blocks[idx], StandardDynamicSwinTransformerBlock):
                    x, k_q_v_dict = self.blocks[idx](x)
                    k_q_v_dict_list.append(k_q_v_dict)
                else:
                    x = self.blocks[idx](x)
                    if torch.isnan(x).any() and False:
        ...
        x = self.classifier(x)
        if self.training:
            return x, block_outputs, k_q_v_dict_list
        return x
    ...
    def config(self):
        return {
            ...
            "final_expand_layer": (
                self.last_conv.final_expand_layer if self.use_v3_head else None
            ),
            "feature_mix_layer": (
                self.last_conv.feature_mix_layer if self.use_v3_head else None
            ),
...
class AttentiveNasQDynamicModel(MyNetwork):
    def __init__(self, supernet, n_classes=1000, bn_param=(0.0, 1e-5), parent=None):
        super(AttentiveNasQDynamicModel, self).__init__()
        ...
        for name in self.stage_names:
            block_cfg = getattr(self.supernet, name)
            self.width_list.append(block_cfg.c)
            self.bit_width_list.append(block_cfg.bit_width)
            if name.startswith("mb"):
                self.depth_list.append(block_cfg.d)
                self.ks_list.append(block_cfg.k)
                self.expand_ratio_list.append(block_cfg.t)
        ...
        self.cfg_candidates = {
            ...
            "kernel_size": self.ks_list,
            "expand_ratio": self.expand_ratio_list,
            "bit_width": self.bit_width_list,
        }

        # first conv layer, including conv, bn, act
        out_channel_list, act_func, stride, bit_width = (
            self.supernet.first_conv.c,
            self.supernet.first_conv.act_func,
            self.supernet.first_conv.s,
            self.supernet.first_conv.bit_width,
        )
        self.first_conv = DynamicQConvBnActLayer(
            in_channel_list=int2list(3),
            out_channel_list=out_channel_list,
            kernel_size=3,
            stride=stride,
        ...
        for stage_id, key in enumerate(self.stage_names[1:-1]):
            ...
            for i in range(n_block):
                ...
                if (
                    ...
                    rescale = 1.0
                    transformer = StandardDynamicQSwinTransformerBlock(
                        feature_dim,
                        input_resolution=(18, 18),
                        num_heads=num_heads[stage_id - 3],
                        window_size=18,
                    ...
                    shortcut = DynamicQShortcutLayer(
                        feature_dim, output_channel, reduction=stride, half_wave=False
                    )
                    blocks.append(
                        MobileInvertedResidualBlock(mobile_inverted_conv, shortcut)
                    )
        ...
        last_channel, act_func, bit_width = (
            self.supernet.last_conv.c,
            self.supernet.last_conv.act_func,
            self.supernet.last_conv.bit_width,
        )
        if not self.use_v3_head:
            self.last_conv = DynamicQConvBnActLayer(
                in_channel_list=feature_dim,
                out_channel_list=last_channel,
                kernel_size=1,
                act_func=act_func,
    ...
    def zero_residual_block_bn_weights(self):
        with torch.no_grad():
            for m in self.modules():
                if isinstance(m, MobileInvertedResidualBlock):
                    if (
                        isinstance(m.mobile_inverted_conv, DynamicMBQConvLayer)
                        and m.shortcut is not None
                    ):
                        m.mobile_inverted_conv.point_linear.bn.bn.weight.zero_()
    ...
    def forward(self, x):
        ...
        # first conv
        """input_=copy.deepcopy(x.clone().detach())
        if torch.isnan(x).any() and False:
                        #print(self.blocks[idx])
                        print("before_first")
                        print(input_)
        ...
        for stage_id, block_idx in enumerate(self.block_group_info):
            ...
            for idx in active_idx:
                ...
                # if idx > active_idx[0]:
                self.blocks[idx].rescale_idx = len(active_idx)
                if isinstance(self.blocks[idx], StandardDynamicQSwinTransformerBlock):
                    x, k_q_v_dict = self.blocks[idx](x)

                    k_q_v_dict_list.append(k_q_v_dict)
                else:
                    p_x = copy.deepcopy(x.clone().detach())
                    x = self.blocks[idx](x)
                    if torch.isnan(x).any() and False:
                        # print(self.blocks[idx])
                        print(self.blocks[idx])
                        print("bith_width", self.blocks[idx].active_bit_width)
                        print("---------------------------------------------------")
                pp_x = copy.deepcopy(x.clone().detach())

                # if self.training:
                #     x = x + torch.randn_like(x) * 5e-3 * x.detach().abs()

            block_names.append(self.blocks[idx])
            block_outputs.append(x)
        ...
        x = self.classifier(x)
        if self.training:
            return x, block_outputs, block_names
        return x, block_outputs, block_names
    ...
    def config(self):
        return {
            "name": AttentiveNasQDynamicModel.__name__,
            "bn": self.get_bn_param(),
            "first_conv": self.first_conv.config,
            "blocks": [block.config for block in self.blocks],
            "last_conv": self.last_conv.config if not self.use_v3_head else None,
    ...
    def set_active_subnet(
        ...
        **kwargs
        ...
        # first conv
        self.first_conv.active_out_channel = width[0]
        self.first_conv.active_bit_width = bit_width[0]

        for stage_id, (c, k, e, d, b) in enumerate(
            zip(width[1:-1], kernel_size, expand_ratio, depth, bit_width)
        ):
            start_idx, end_idx = min(self.block_group_info[stage_id]), max(
                self.block_group_info[stage_id]
            )
            ...
            for block_id in range(start_idx, start_idx + d):
                block = self.blocks[block_id]
                block.active_bit_width = bit_width[stage_id]
                # block output channels

                if block.mobile_inverted_conv != None:
                    block.mobile_inverted_conv.active_out_channel = c
                    block.mobile_inverted_conv.active_bit_width = block.active_bit_width
                    if block.shortcut is not None:
                        block.shortcut.active_out_channel = c
                        block.shortcut.active_bit_width = block.active_bit_width
                    ...
                    pass
                """if block.transformer !=None:
                    block.transfomer.active_bit_width=bit_width[stage_id]
                else:
                    pass"""
                if isinstance(block, StandardDynamicQSwinTransformerBlock):
                    block.active_bit_width = bit_width[stage_id]
                    # if block_id == start_idx:
                    #     block.out_dim = c
        # IRBlocks repated times
    ...
    def get_active_subnet_settings(self):
        r = self.active_resolution
        width, depth, kernel_size, expand_ratio, bit_width = [], [], [], [], []

        # first conv
        width.append(self.first_conv.active_out_channel)
        bit_width.append(self.first_conv.active_bit_width)
        for stage_id in range(len(self.block_group_info)):
            start_idx = min(self.block_group_info[stage_id])
            block = self.blocks[start_idx]  # first block
            try:
                ...
                kernel_size.append(block.mobile_inverted_conv.active_kernel_size)
                expand_ratio.append(block.mobile_inverted_conv.active_expand_ratio)
                bit_width.append(block.mobile_inverted_conv.active_bit_width)
            except:
                pass
                """
                # double check
    ...
    def sample_min_subnet(self, quantized=False):
        return self._sample_active_subnet(min_net=True, quantized=quantized)

    def sample_max_subnet(self, quantized=False):
        return self._sample_active_subnet(max_net=True, quantized=quantized)

    def sample_active_subnet(self, compute_flops=False, quantized=False):
        cfg = self._sample_active_subnet(False, False, quantized=quantized)
        if compute_flops:
            cfg["flops"] = self.compute_active_subnet_flops()
        return cfg
    ...
    def _sample_active_subnet(self, min_net=False, max_net=False, quantized=False):
        sample_cfg = lambda candidates, sample_min, sample_max: (
            min(candidates)
            if sample_min
            else (max(candidates) if sample_max else random.choice(candidates))
        ...
        for k in ["width", "depth", "kernel_size", "expand_ratio", "bit_width"]:
            cfg[k] = []
            for vv in self.cfg_candidates[k]:
                cfg[k].append(sample_cfg(int2list(vv), min_net, max_net))

        fp_bit_width = []
        for i in range(len(cfg["bit_width"])):
            fp_bit_width.append(-1)
        ...
        """self.set_active_subnet(
            cfg['resolution'], cfg['width'], cfg['depth'], cfg['kernel_size'], cfg['expand_ratio'], fp_bit_width #cfg['bit_width']
        )"""
        return cfg

    def set_quantization_policy(
        self,
        pw_w_bits_setting=None,
        pw_a_bits_setting=None,
        dw_w_bits_setting=None,
        ...
        for block, pw_w, pw_a, dw_w, dw_a in zip(
            self.blocks[1:],
            pw_w_bits_setting,
            pw_a_bits_setting,
            dw_w_bits_setting,
            ...
            block.mobile_inverted_conv.set_quantization_policy(
                pw_w_bit=pw_w, pw_a_bit=pw_a, dw_w_bit=dw_w, dw_a_bit=dw_a
            )
    ...
    def sample_quantization_policy(self):
        ...
        self.set_quantization_policy(
            pw_w_bits_setting, pw_a_bits_setting, dw_w_bits_setting, dw_a_bits_setting
        )
