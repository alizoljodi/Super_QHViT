...
from .static_layers import SELayer, QSELayer
...
class DynamicSeparableQConv2d(QModule):
    ...
    def __init__(
        ...
        super(DynamicSeparableQConv2d, self).__init__(
            w_bit=w_bit, a_bit=a_bit, half_wave=True
        )
        ...
        self.dilation = dilation
        self.channels_per_group = channels_per_group
        max_kernel = max(self.kernel_size_list)
        self.parent = parent
...
class DynamicPointQConv2d(QModule):
    def __init__(
        ...
        self.max_in_channel = max_in_channels
        self.max_out_channel = max_out_channels
        self.kernel_size = kernel_size
        self.stride = stride
        self.dilation = dilation
        self.parent = parent
        ...
        self.conv = nn.Conv2d(
            self.max_in_channel,
            self.max_out_channel,
            self.kernel_size,
            self.stride,
            bias=False,
        )

        self.active_out_channel = self.max_out_channel
...
class DynamicBatchNorm2d(nn.Module):
    ...
    def __init__(self, max_feature_dim, parent=None):
    ...
    def forward(self, x):
        ...
        if not self.need_sync:
            ...
            if torch.isnan(x).any():
                print("here")
            """print("......................")
            print(bn.running_mean[:feature_dim])
            print(bn.running_var[:feature_dim])
            print(bn.weight[:feature_dim])
            print(bn.bias[:feature_dim])
            print('grad:',x.grad)
...
class DynamicSE(SELayer):
    def __init__(self, max_channel, parent=None):
...
class DynamicQSE(QSELayer):
    def __init__(self, max_channel, w_bit=-1, a_bit=-1, half_wave=True, parent=None):
        super(DynamicQSE, self).__init__(
            max_channel, w_bit=w_bit, a_bit=a_bit, half_wave=half_wave
        )
