...
from misc.quantize_utils import QModule
...
class DynamicQMlp(QModule):
    def __init__(
        ...
        super(DynamicQMlp, self).__init__(w_bit=w_bit, a_bit=a_bit, half_wave=half_wave)
...
class DynamicWindowAttention(MyModule):
    ...
    def forward(
        self, x, mask=None, writer=None, batch_idx=None, type_=None, block_id=None
    ):
        """
        Args:
        ...
        if writer is not None and batch_idx is not None:
            writer.add_histogram(
                "q" + type_ + str(self.__class__) + str(block_id),
                q.detach().cpu().numpy(),
                global_step=batch_idx,
                bins="auto",
            )
            writer.add_histogram(
                "k" + type_ + str(self.__class__) + str(block_id),
                k.detach().cpu().numpy(),
                global_step=batch_idx,
                bins="auto",
            )
...
class DynamicQWindowAttention(QModule):
    def __init__(
        self,
        dim_list,
        window_size,
        ...
        super(DynamicQWindowAttention, self).__init__(
        ...
        # DynamicLinear(max_in_features=max(self.dim_list), max_out_features=max(self.dim_list), bias=qkv_bias)
        # DynamicQLinear(max_in_features=max(self.dim_list), max_out_features=max(self.dim_list), bias=qkv_bias,w_bit=w_bit,a_bit=a_bit,half_wave=half_wave)

        # DynamicLinear(max_in_features=max(self.dim_list), max_out_features=max(self.dim_list), bias=qkv_bias)

        self.k = DynamicQLinear(
            max_in_features=max(self.dim_list),
            max_out_features=max(self.dim_list),
            bias=qkv_bias,
            w_bit=w_bit,
    ...
    def forward(
        ...
        weight = self.vconv.weight[: C * self.expan_ratio, :, :, :].contiguous()
        ...
        # attn=self._quantize_activation(attn)
    ...
    def config(self):
        return {
            "name": DynamicQWindowAttention.__name__,
            "dim_list": self.dim_list,
            "window_size": self.window_size,
        }
...
class DynamicSwinTransformerBlock(MyModule):
    ...
    def forward(self, x, writer=None, batch_idx=None, block_id=None):
        ...
        if H > 18 and self.shift:
            ...
            attn_windows, k_q_v_dict = self.attn(
                x_windows, writer, batch_idx, "shifted", block_id
            )  # , mask=self.attn_mask)
        ...
        return x.reshape(B, H, W, C).permute(0, 3, 1, 2), k_q_v_dict
...
class DynamicQSwinTransformerBlock(QModule):
    def __init__(
        self,
        dim_list,
        input_resolution,
        ...
        super(DynamicQSwinTransformerBlock, self).__init__(
            w_bit=max(bit_width), a_bit=max(bit_width), half_wave=half_wave
        )
        ...
        """self.attn = DynamicWindowAttention(
            ...
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=atten_drop, proj_drop=drop, act_layer=act_layer, downsample=downsample)"""
        self.attn = DynamicQWindowAttention(
            dim_list,
            window_size=to_2tuple(window_size),
            num_heads=num_heads,
            qkv_bias=qkv_bias,
        ...
        # self.mlp1 = DynamicQMlp(out_features=self.dim, hidden_features_list=[int(1. * self.dim)], act_layer=act_layer)#, drop=drop)

        self.mlp2 = DynamicQMlp(
            out_features=self.dim,
            hidden_features_list=[int(1.0 * self.dim)],
            act_layer=act_layer,
            w_bit=self.active_bit_width,
        ...
        # self.mlp2 = DynamicQMlp(out_features=self.dim, hidden_features_list=[int(1. * self.dim)], act_layer=act_layer)#, drop=drop)
    ...
    def forward(self, x, writer=None, batch_idx=None, block_id=None):
        ...
        norm1_weight = self.norm1.weight[: x.shape[-1]]  # .contiguous()
        norm1_bias = self.norm1.bias[: x.shape[-1]]
        # norm1_input,norm1_weight,norm1_bias=self._quantize(x,norm1_weight,norm1_bias)
        x = F.layer_norm(
            x,
            x.shape[1:],
            norm1_weight.expand(x.shape[1:]),
            norm1_bias.expand(x.shape[1:]),
        )
    ...
    @staticmethod
    def name():
        return "DynamicQSwinTransformerBlock"
    ...
    """def get_active_subnet(self, in_features, preserve_weight=True):
        ...
        sub_layer = DynamicQSwinTransformerBlock(self.dim_list,self.input_resolution, self.num_heads, self.window_size, self.shift_size, self.mlp_ratio,
                bit_width=self.active_bit_width,half_wave=True,parent=None):
        sub_layer = sub_layer.to(get_net_device(self))
        if not preserve_weight:
            return sub_layer
        
        sub_layer.attn.weight.data.copy_
